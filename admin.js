...
function getDefaultData() {
  return {
    ...
    links: {
      tiktok: { label: "TikTok", hint: "Hauptplattform" },
      discord: { label: "Discord", hint: "Community Server" },
      twitch: { label: "Twitch", hint: "Streams" },
      youtube: { label: "YouTube", hint: "" },
      telegram: { label: "Telegram", hint: "@Liro025" },
      vrchat: { label: "VRChat", hint: "Liro0" },
      kofi: { label: "Ko-fi", hint: "Ko-fi Seite" }
    },
    collabs: [],
    pages: {
      about: { title: "Über mich", subtitle: "", content: "" }
    },
    blogPosts: []
  };
}
...
/* ===== TAB 6: Blog (NEUER EDITOR mit Blöcke) ===== */
...
function removeBlock(blockId) {
  blogBlocks = blogBlocks.filter(b => b.id !== blockId);
  renderBlocks();
}
...
function renderBlocks() {
  ...
  blogBlocks.forEach((block, index) => {
    ...
    blockDiv.className = "blog-block";

    if (block.type === "text") {
      blockDiv.innerHTML = `
        <div class="blog-block__header">
          <span class="blog-block__type">📝 Text-Block ${index + 1}</span>
          <button type="button" class="blog-block__remove" onclick="removeBlock('${block.id}')">✕</button>
        </div>
        <div class="blog-block-text">
          <textarea placeholder="Schreib deinen Text hier..." onchange="updateBlockContent('${block.id}', 'content', this.value)">${block.content}</textarea>
        </div>
      `;
    }
    ...
    if (block.type === "heading") {
      blockDiv.innerHTML = `
        <div class="blog-block__header">
          <span class="blog-block__type">📌 Überschrift ${index + 1}</span>
          <button type="button" class="blog-block__remove" onclick="removeBlock('${block.id}')">✕</button>
        </div>
        <div class="blog-block-heading">
          <input type="text" placeholder="Überschriften-Text..." value="${block.content}" onchange="updateBlockContent('${block.id}', 'content', this.value)">
        </div>
      `;
    }
    ...
    if (block.type === "image") {
      blockDiv.innerHTML = `
        <div class="blog-block__header">
          <span class="blog-block__type">🖼️ Bild-Block ${index + 1}</span>
          <button type="button" class="blog-block__remove" onclick="removeBlock('${block.id}')">✕</button>
        </div>
        <div class="blog-block-image">
          ${block.url ? `<div class="blog-block-image__preview"><img src="${block.url}" alt="preview"></div>` : ''}
          <div class="blog-block-image__upload">
            <div class="upload-zone-small" onclick="document.getElementById('blog-img-upload-${block.id}').click()">
              📤 Klick zum Upload
            </div>
            <input type="file" id="blog-img-upload-${block.id}" accept="image/*" style="display: none;" onchange="handleBlogImageUpload('${block.id}', this)">
          </div>
          <div>
            <label style="font-size: 0.85rem;">Position:</label>
            <select onchange="updateBlockContent('${block.id}', 'position', this.value)">
              <option value="left" ${block.position === 'left' ? 'selected' : ''}>Links</option>
              <option value="right" ${block.position === 'right' ? 'selected' : ''}>Rechts</option>
              <option value="full" ${block.position === 'full' ? 'selected' : ''}>Vollbreite</option>
            </select>
          </div>
          <div>
            <label style="font-size: 0.85rem;">Breite (%):</label>
            <input type="number" min="20" max="100" value="${block.width}" onchange="updateBlockContent('${block.id}', 'width', this.value)">
          </div>
        </div>
      `;
    }
...
function handleBlogImageUpload(blockId, input) {
...
document.getElementById("create-blog-btn")?.addEventListener("click", async () => {
  ...
  if (!title || !date || !excerpt) {
    alert("❌ Bitte Titel, Datum und Excerpt ausfüllen!");
    return;
  }

  if (blogBlocks.length === 0) {
    alert("❌ Füge mindestens einen Block hinzu!");
    return;
  }
  ...
  try {
    ...
    const newId = Math.max(...siteData.blogPosts.map(p => p.id || 0), 0) + 1;

    const processedBlocks = blogBlocks.map(block => {
      if (block.type === "image") {
        return {
          type: "image",
          url: block.url,
...
function editBlogPost(id) {
  ...
  try {
    blogBlocks = JSON.parse(post.content);
  } catch (e) {
    blogBlocks = [];
  }
  renderBlocks();
...
document.getElementById("add-text-block-btn")?.addEventListener("click", addTextBlock);
document.getElementById("add-heading-block-btn")?.addEventListener("click", addHeadingBlock);
document.getElementById("add-image-block-btn")?.addEventListener("click", addImageBlock);
